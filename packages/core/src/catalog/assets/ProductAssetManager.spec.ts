...
import {
	...
	type ProductAssetLoader
} from './ProductAssetManager.js';

const resource: ProductAssetResource = {
...
it('evicts idle bytes while preserving live instances even above the budget', async () => {
	...
	const manager = new ProductAssetManager({ loader, maxIdleEntries: 6, maxIdleBytes: 1 });
...
it('does not clone a cached resource during motion or retain a cancelled queued instance', async () => {
	...
	const scene = new THREE.Group();
	scene.add(new THREE.Mesh(new THREE.BoxGeometry(), new THREE.MeshStandardMaterial()));
	const manager = new ProductAssetManager({
...
function createLoader() {
	const requests: Array<ReturnType<typeof deferred<LoadedProductAsset>> & { signal: AbortSignal }> =
		[];
	const load = vi.fn<ProductAssetLoader['load']>((_resource, { signal }) => {
		const request = { ...deferred<LoadedProductAsset>(), signal };
...
describe('ProductAssetManager', () => {
	it('deduplicates loads and isolates mutable instance materials without disposing shared resources', async () => {
		...
		const manager = new ProductAssetManager({ loader });
	...
	it('lets instances outlive leases and runtime closure without freeing their shared geometry', async () => {
		const source = asset();
		const manager = new ProductAssetManager({ loader: { load: async () => source } });
		const lease = manager.acquire(resource);
		const instance = await lease.createInstance();
	...
	it('never requests high implicitly and keeps resource revisions independent', async () => {
		const load = vi.fn<ProductAssetLoader['load']>().mockImplementation(async () => asset());
		const manager = new ProductAssetManager({ loader: { load } });
