...
import {
	createSpatialElementAssetInstance,
	disposeLoadedSpatialElementAsset,
	type LoadedSpatialElementAsset,
	type SpatialElementAssetInstance
} from './spatialElementAssetResources.js';

export type { LoadedSpatialElementAsset, SpatialElementAssetInstance } from './spatialElementAssetResources.js';

export interface SpatialElementAssetLoader {
	load(
		resource: SpatialElementAssetResource,
		options: { signal: AbortSignal }
	): Promise<LoadedSpatialElementAsset>;
	dispose?(): void;
}

export interface SpatialElementAssetLease {
	readonly key: string;
	/** Decoded CPU readiness only; the renderer separately establishes GPU readiness. */
	readonly ready: Promise<LoadedSpatialElementAsset>;
	/** A live instance pins shared resources independently of this lease. */
	createInstance(): Promise<SpatialElementAssetInstance>;
	release(): void;
}
...
interface AssetEntry {
	...
	promise: Promise<LoadedSpatialElementAsset>;
	resolve: (asset: LoadedSpatialElementAsset) => void;
...
export function getSpatialElementAssetResourceKey(resource: SpatialElementAssetResource) {
	if (!resource.url || !resource.revision) {
		throw new Error('SpatialElement asset requests require a URL and revision');
	}
...
export class SpatialElementAssetManager {
	...
	constructor(
		options: {
			loader?: SpatialElementAssetLoader;
		...
		this.loader = options.loader ?? new GltfSpatialElementAssetLoader(options.preparationGate, options.dracoDecoderPath);
	...
	setDracoDecoderPath(path: string) {
		if (this.loader instanceof GltfSpatialElementAssetLoader) this.loader.setDracoDecoderPath(path);
	}

	acquire(
		resource: SpatialElementAssetResource,
		options: { signal?: AbortSignal } = {}
	): SpatialElementAssetLease {
		if (this.disposed) throw new Error('SpatialElementAssetManager has been disposed');
		if (options.signal?.aborted) throw abortError();
		const key = getSpatialElementAssetResourceKey(resource);
		...
		const ready = new Promise<LoadedSpatialElementAsset>((resolve, reject) => {
			rejectReady = reject;
			entry.promise.then(
				(asset) => {
		...
		return {
			...
			createInstance: async () => {
				...
				const create = () => {
					if (released || entry.retired) throw abortError();
					const instance = createSpatialElementAssetInstance(asset, () => {
						entry.instances -= 1;
						entry.lastUsed = ++this.clock;
						this.releaseUnused(entry);
					});
	...
	invalidate(resource: SpatialElementAssetResource) {
		const entry = this.cache.get(getSpatialElementAssetResourceKey(resource));
	...
	private startLoad(key: string, resource: SpatialElementAssetResource) {
		let resolve!: AssetEntry['resolve'];
		let reject!: AssetEntry['reject'];
		const promise = new Promise<LoadedSpatialElementAsset>((accept, fail) => {
			resolve = accept;
			reject = fail;
		});
		...
		void Promise.resolve()
			...
			.then((asset) => {
				if (entry.retired || this.cache.get(key) !== entry) {
					disposeLoadedSpatialElementAsset(asset);
					entry.status = 'failed';
					entry.reject(abortError('A newer spatialElement asset generation replaced this load'));
					return;
				}
				entry.asset = asset;
...
export class GltfSpatialElementAssetLoader implements SpatialElementAssetLoader {
 ...
 setDracoDecoderPath(path: string) {
  ...
  this.decoderPath = path;
	...
	async load(resource: SpatialElementAssetResource, { signal }: { signal: AbortSignal }) {
		...
		if (!response.ok) throw new Error(`Unable to load spatialElement asset (${response.status})`);
