...
import {
	createLegacySpatialElementAssetManifest,
	getSpatialElementEntityKey,
	getRepresentationResource,
	SpatialElementEntityStore
} from '../spatialElementAssets.js';

describe('spatialElement asset identity and migration', () => {
	it('retains logical spatialElement identity when its manifest changes and avoids compound-key collisions', () => {
		const store = new SpatialElementEntityStore();
		const first = store.get('demo', 'column');
		const manifest = createLegacySpatialElementAssetManifest('demo', 'column', {
		...
		expect(getSpatialElementEntityKey('a:b', 'c')).not.toBe(getSpatialElementEntityKey('a', 'b:c'));
	});
