...
export interface ProductDockRect {
	left: number;
	top: number;
	width: number;
	height: number;
...
export const PRODUCT_TABS_DOCK_SEAM_OVERLAP = 1;

/** Resolves the fixed product-header rectangle for a viewport. */
export function getProductDockDestination(
...
export function getMinimapDockDestination(source: ProductDockRect): ProductDockRect {
	return {
		...source,
		top: 0
	};
...
export function getCompactMinimapDockDestination(
	source: ProductDockRect,
	header: ProductDockRect,
	viewportWidth: number,
	inset = 4
): ProductDockRect {
...
export function getProductTabsDockDestination(
	source: ProductDockRect,
	header: ProductDockRect,
	viewportWidth: number,
	inset = 8,
	seamOverlap = PRODUCT_TABS_DOCK_SEAM_OVERLAP
): ProductDockRect {
	const safeInset = Math.max(inset, 0);
...
export function getProductTabsDockBreakpoint(sourceDocumentTop: number, dockTop: number) {
	return Math.max(sourceDocumentTop - dockTop, 0);
}

/** Tabs only dock after the product header is available as their visible anchor. */
export function getProductTabsDockTarget(
	scrollY: number,
	breakpoint: number,
...
export function getProductTabsDockProgress(scrollY: number, breakpoint: number, distance = 24) {
...
export function getProductSectionScrollOffset(tabs: ProductDockRect, gap = 16) {
	return Math.max(tabs.top + tabs.height + Math.max(gap, 0), 0);
}

/**
 * Makes the docked product overhang as deliberately as the minimap is taller than
 * the header, without requiring product-specific optical tuning.
 */
export function getDockedMinimapModelScale(
	minimap: ProductDockRect,
	header: ProductDockRect,
...
export function getProductDockBreakpoint(minimapDocumentTop: number, minimapDockTop: number) {
 ...
 */
export function getProductDockPresentationProgress(
	scrollY: number,
...
export function getProductDockTarget(
	scrollY: number,
	breakpoint: number,
