...
const MINIMAP_ANCHOR = '[data-product-dock-minimap-anchor]';
const PANEL_FRAME = '[data-stage-panel-fallback]';
const STICKY_HEADER = '[data-product-sticky-header]';
const STICKY_MINIMAP_ACTION = '[data-product-sticky-minimap-action]';
const TABS_ANCHOR = '[data-product-tabs-panel-anchor]';
const TABS_LIST = '[data-product-tab-list]';
const TABS_LINKS = '[data-product-tab-links]';
const STICKY_TABS = '[data-product-sticky-tabs]';
const PRODUCT_LAYOUT_ROOT = '.stage';
const DEPARTING_CONTENT =
	'[data-product-hero-panel-content], [data-product-departing-media-content]';
 ...
 * The normal product summary remains in document flow and simply leaves with the hero.
 */
export class ProductDockController {
	...
	private minimapDock?: ProductDockRect;
	private tabsDock?: ProductDockRect;
	...
	constructor(private readonly root: HTMLElement) {
		this.layoutRoot = root.closest<HTMLElement>(PRODUCT_LAYOUT_ROOT) ?? undefined;
	}
	...
	destroy() {
		...
		this.root.removeAttribute('data-product-dock-active');
		this.root.removeAttribute('data-product-tabs-dock-active');
		this.root.style.removeProperty('--product-section-scroll-offset');
		(this.layoutRoot ?? this.root).style.removeProperty('--product-content-inline-inset');
	}
	...
	private readonly handleTabFocusIn = (event: FocusEvent) => {
		...
		const tabId = event.target.closest<HTMLElement>('[data-product-tab-id]')?.dataset.productTabId;
		if (!tabId) return;
	...
	private measure() {
		...
		this.breakpoint = getProductDockBreakpoint(
		...
		this.stickyHeader.style.setProperty('--product-sticky-minimap-reserve', `${reserve}px`);
		(this.layoutRoot ?? this.root).style.setProperty(
			'--product-content-inline-inset',
		...
		if (this.tabsFrame && this.stickyTabs) {
			...
			this.tabsDock = getProductTabsDockDestination(tabsRect, headerRect, window.innerWidth);
			this.tabsBreakpoint = getProductTabsDockBreakpoint(
			...
			this.root.style.setProperty(
				'--product-section-scroll-offset',
				`${getProductSectionScrollOffset(this.tabsDock)}px`
			);
		}
	}
	...
	private update() {
		...
		const presentationProgress = getProductDockPresentationProgress(
			this.scrollY,
			this.presentationEndScrollY
		);
		const docked = getProductDockTarget(this.scrollY, this.breakpoint, this.docked);
		const tabsDocked = getProductTabsDockTarget(
			this.scrollY,
			this.tabsBreakpoint,
			docked,
			this.tabsDocked
	...
	private applyState(docked: boolean) {
		...
		if (changed) {
			...
			this.root.toggleAttribute('data-product-dock-active', docked);
		}
	...
	private applyTabsState(docked: boolean) {
		...
		this.root.toggleAttribute('data-product-tabs-dock-active', docked);
	}
	...
	private applyTabsPresentation(docked: boolean) {
		...
		const progress = docked
			? reducedMotion
				? 1
				: getProductTabsDockProgress(this.scrollY, this.tabsBreakpoint, TABS_DOCK_MORPH_DISTANCE)
			: 0;
		...
		this.stickyTabs.setAttribute('data-product-tabs-dock-progress', progress.toFixed(3));
	}
	...
	private applyHeaderPresentation(progress: number) {
		...
		this.stickyHeader.style.setProperty('--product-sticky-header-progress', value);
		this.stickyHeader.setAttribute('data-product-dock-progress', value);
		this.stickyHeader.toggleAttribute('data-presented', clampedProgress > 0);
	}

	private restoreHeaderPresentation() {
		if (!this.stickyHeader) return;
		this.stickyHeader.style.removeProperty('--product-sticky-header-progress');
		this.stickyHeader.removeAttribute('data-product-dock-progress');
	...
	private setTabsActive(active: boolean) {
		...
		const focusedTabId =
			document.activeElement instanceof HTMLElement && outgoingList.contains(document.activeElement)
				? document.activeElement.closest<HTMLElement>('[data-product-tab-id]')?.dataset.productTabId
		...
		this.sourceTabsList.toggleAttribute('data-product-tab-list-hidden', active);
		...
		const focusMatchingTab = (retry = true) => {
			...
			const matchingTab = Array.from(
				incomingList.querySelectorAll<HTMLElement>('[data-product-tab-id]')
			).find((element) => element.dataset.productTabId === focusedTabId);
	...
	private restoreStickyTabs() {
		...
		this.stickyTabs.removeAttribute('data-product-tabs-dock-progress');
...
function applyFixedRect(frame: HTMLElement, rect: ProductDockRect) {
...
function applyViewportFixedRect(element: HTMLElement, rect: ProductDockRect) {
...
function toRect(rect: DOMRect): ProductDockRect {
	return {
		left: rect.left,
		top: rect.top,
		width: rect.width,
