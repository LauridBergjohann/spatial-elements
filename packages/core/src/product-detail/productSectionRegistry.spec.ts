...
import { ProductSectionRegistry } from './productSectionRegistry.js';

describe('ProductSectionRegistry', () => {
	it('keeps sections in render order and emits immutable navigation snapshots', () => {
		const onChange = vi.fn();
		const registry = new ProductSectionRegistry(onChange);
	...
	it('updates navigation metadata without changing section order', () => {
		const registry = new ProductSectionRegistry();
