...
/** Separate scene captures preserve authored alpha without sorting two translucent product meshes. */
export class ProductRefinement {
	getRenderTargets() {
	...
	render(
		...
		if (!destination && !this.overlay)
			throw new Error('Product refinement requires an owned scene capture');
