...
import { prepareSpatialElementFrame } from './spatialElementFrame.js';
...
interface HeroTransfer {
	...
	refinement?: SpatialElementRefinement;
...
export class SpatialElementPresentationController {
	getRenderTargets() {
	...
	private instance?: SpatialElementAssetInstance;
	private high?: PreparedHigh;
	private overrides = new Set<THREE.Material>();
	private blend?: SpatialElementRefinement;
	private movingBlend?: SpatialElementRefinement;
	private retainedBlend?: SpatialElementRefinement;
	private state: RefinementState = 'unavailable';
	private frames = 0;

	constructor(
		private readonly assets: Pick<SpatialElementAssetManager, 'acquire'>,
	...
	finishHandoff(synchronous: boolean) {
		if (synchronous && this.high && this.state !== 'high') this.blend = new SpatialElementRefinement(true);
	...
	restoreLowInstance(low?: SpatialElementAssetInstance) {
		this.instance = low;
	}
	...
	async load(profile: Profile) {
		...
		let instance: SpatialElementAssetInstance;
		try {
		...
		if (profile.lodPair)
			this.bounds = prepareSpatialElementFrame(model, instance.scene, profile.lodPair, profile.model);
		else {
	...
	startHigh() {
		...
		void (async () => {
			...
			try {
				...
				prepareSpatialElementFrame(container, instance.scene, pair, profile.model);
				if (
					getMeshBounds(
						container,
