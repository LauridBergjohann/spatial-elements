...
function instance() {
	...
	return { scene, dispose: vi.fn() } as unknown as ProductAssetInstance;
}
function fixture() {
	const pending: ((value: ProductAssetInstance) => void)[] = [];
	const release = vi.fn();
	const assets = {
		acquire: vi.fn(() => ({
			createInstance: () => new Promise<ProductAssetInstance>((resolve) => pending.push(resolve)),
			release
		}))
	} as unknown as Pick<ProductAssetManager, 'acquire'>;
	return { owner: new ProductPresentationController(assets, vi.fn()), pending, release };
}
...
test('interrupted prepared High is released exactly once while Low remains usable', async () => {
	...
	const high = instance();
	owner.adoptHigh(
		{ instance: high, pose: high.scene, overrides: new Set(), blend: new ProductRefinement(true) },
		false
	);
	owner.cancelHigh();
	owner.cancelHigh();
