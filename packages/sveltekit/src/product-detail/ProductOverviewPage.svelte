<script lang="ts">
	import ContentPage from './ContentPage.svelte';
	import ListSection from './ListSection.svelte';
	import Section from './Section.svelte';
	import { useCatalogNavigation } from '../catalog/catalogNavigation.js';
	import type { ProductOverviewItem, ContentPageData } from '@spatial-elements/core/product-detail/types';
	let {
		products,
		sectionLink,
		...page
	}: ContentPageData & {
		products: ProductOverviewItem[];
		sectionLink?: { href: string; label: string };
	} = $props();
	const { href } = useCatalogNavigation();
</script>

<ContentPage {page}>
	<ListSection section={{ id: 'products', title: 'Products' }} list={products} />
	{#if sectionLink}<Section section={{ id: 'more', title: 'More information' }}
			><a href={href(sectionLink.href)}>{sectionLink.label}</a></Section
		>{/if}
</ContentPage>
