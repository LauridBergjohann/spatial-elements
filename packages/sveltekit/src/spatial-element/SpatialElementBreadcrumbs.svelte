<script lang="ts">
	...
	let {
		items,
		transition = 'enter'
	}: { items: SpatialElementBreadcrumb[]; transition?: 'enter' | 'exit' } = $props();
</script>
...
<style>
	.breadcrumbs {
		...
		margin: 8px 0 12px calc(50% - 50vw + var(--spatial-element-content-inline-inset));
		color: color-mix(in srgb, var(--spatial-element-body) 72%, transparent);
		font-size: 13px;
		line-height: 16px;
	}
