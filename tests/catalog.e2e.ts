...
for (const view of ['list','carousel','mixed']) test(view+' enhances and survives detail/history navigation',async({page})=>{
 ...
 const target=view==='carousel'?page.locator('[data-carousel-item="column"] .information').first():page.locator('[data-catalog-card][data-product-id="column"]').first();
 ...
 await target.click();await expect(page).toHaveURL(/products\/column/);
 ...
 await expect(page.locator('[data-catalog-transition]')).toHaveCount(0);
 await page.goForward();await expect(page).toHaveURL(/products\/column/);
...
test('content anchor navigates into the detail section',async({page})=>{
 ...
 await expect(page).toHaveURL(/products\/orb#features/);await expect(page.locator('#features')).toBeInViewport();
});
test('HTML remains useful without JavaScript',async({browser})=>{
 ...
 await page.locator('[data-catalog-card][data-product-id="orb"]').click();await expect(page).toHaveURL(/products\/orb/);await expect(page.getByRole('heading',{name:'Orb',exact:true}).first()).toBeVisible();await context.close();
});
test('GPU unavailable retains posters and native navigation',async({page})=>{
 await page.addInitScript(()=>Object.defineProperty(navigator,'gpu',{value:undefined,configurable:true}));await page.goto('/demo/categories/list');await expect(page.locator('.stage')).toHaveAttribute('data-stage-state','fallback');await page.locator('[data-catalog-card][data-product-id="ring"]').click();await expect(page).toHaveURL(/products\/ring/);
});
